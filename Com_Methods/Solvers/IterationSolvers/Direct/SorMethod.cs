...
{
    class SorMethod : IIterationSolver
    {
        public SorMethod(int maxIterationCount, double eps)
        {
        ...
        public Vector Solve(Matrix matrixA, Vector vectorF, double relaxationFactorW)
        {
            double normX;
            ...
            do
            {
                ...
                for (int i = 0; i < result.N; i++)
                {
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        F_Ax -= matrixA.Elem[i][j] * resultNext.Elem[j];
                    }
                    ...
                    resultNext.Elem[i] = (1.0d - relaxationFactorW) * result.Elem[i] + relaxationFactorW * F_Ax;
